...
public class opFacturar {

    public opFacturar() {
    }

    public ArrayList<reserva> reservasSinFacturar(usuario usuario, Connection conexion) throws SQLException {

        ArrayList<reserva> reservas = new ArrayList();

        String sql = "select reservaOcupacion from ocupacion where facturacionOcupacion LIKE 'N' group by reservaOcupacion";
        ...
        if (!idsReserva.isEmpty()) {
            String sql2 = "SELECT reserva.* FROM reserva WHERE idReserva = ? AND usuarioReserva = ?";
    ...
    public ArrayList<reserva> completarPasajeros(ArrayList<reserva> reservas, Connection conexion) throws SQLException {
        for (reserva reserva : reservas) {
            String sql = "SELECT pasajeroOcupacion FROM ocupacion WHERE reservaOcupacion = ? GROUP BY pasajeroOcupacion";
            PreparedStatement estado = conexion.prepareStatement(sql);
            ...
            while (resultado.next()) {
                ...
                sql = "SELECT * FROM ocupacion WHERE reservaOcupacion = ? AND pasajeroOcupacion = ? AND tipoOcupacion LIKE ?";
                PreparedStatement estado3 = conexion.prepareStatement(sql);
                ...
                if (resultado3.next()) {
                    ...
                    sql = "SELECT s.* FROM servicio s, servicioreserva sr WHERE sr.ocupacionRS = ? AND sr.servicioRS = s.idServicio";
                    PreparedStatement estado4 = conexion.prepareStatement(sql);
    ...
    public void facturarIda(reserva reserva, Connection conexion) throws SQLException {
        String sql = "UPDATE ocupacion SET facturacionOcupacion = ? WHERE reservaOcupacion = ? AND tipoOcupacion LIKE 'ida'";
        PreparedStatement estado = conexion.prepareStatement(sql);
        estado.setString(1, "Y");
        estado.setInt(2, reserva.getId());
        estado.executeUpdate();
        estado.close();
    }

    public void facturarVuelta(reserva reserva, Connection conexion) throws SQLException {
        String sql = "UPDATE ocupacion SET facturacionOcupacion = ? WHERE reservaOcupacion = ? AND tipoOcupacion LIKE 'vuelta'";
        PreparedStatement estado = conexion.prepareStatement(sql);
        estado.setString(1, "Y");
        estado.setInt(2, reserva.getId());
        estado.executeUpdate();
    ...
    public void facturarAmbas(reserva reserva, Connection conexion) throws SQLException {
        String sql = "UPDATE ocupacion SET facturacionOcupacion = ? WHERE reservaOcupacion = ?";
        PreparedStatement estado = conexion.prepareStatement(sql);
        estado.setString(1, "Y");
        estado.setInt(2, reserva.getId());
        estado.executeUpdate();
